        ...
        class ClearTag : MonoBehaviour
        {
            static void Prefix(GorillaTagManager __instance)
            {
                ...
                if (!PhotonNetwork.CurrentRoom.IsVisible || !PhotonNetwork.InRoom)
                {
                    ...
                    if (secondaryDown)
                    {
                        if (__instance.currentIt == PhotonNetwork.LocalPlayer || __instance.currentInfected.Contains(PhotonNetwork.LocalPlayer));
                        {
                            int num = 0;
                            foreach (Photon.Realtime.Player player in PhotonNetwork.PlayerList)
                            {
                                ...
                                if (num < 4 && __instance.isCurrentlyTag == true)
                                {
                                ...
                                if (num >= 4 && __instance.isCurrentlyTag == false)
                                {

                                    __instance.ClearInfectionState();
                                    __instance.lastInfectedPlayer = null;
                                    __instance.currentInfected = null;
                                    ExitGames.Client.Photon.Hashtable hashtable = new ExitGames.Client.Photon.Hashtable();
                                    hashtable.Add("lastInfectedPlayer", null);
                                    __instance.currentRoom.SetCustomProperties(hashtable, null, null);
                            }
                        }
                    }
                    if ((primaryDown && __instance.currentIt != PhotonNetwork.LocalPlayer) && (!__instance.currentInfected.Contains(PhotonNetwork.LocalPlayer)))
                    {
                        int num = 0;
                        foreach (Photon.Realtime.Player player in PhotonNetwork.PlayerList)
                        {
